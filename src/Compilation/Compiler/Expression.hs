...
type ExpAccumulator a = Accumulator (Int, [CExtDecl]) a

insertExtDecl :: CExtDecl -> ExpAccumulator ()
insertExtDecl newDecls = do
    updateAccumulator (\ (x, decls) -> (x + 1, newDecls : decls))
    return ()

translateExp :: CompilationUnit' -> LocalInformation -> Exp' -> ExpAccumulator CExpr
...
translateExp unit locals (InstanceCreation' (ClassType' name) args) = do
    let cName = cIdent (head name)
    cArgs <- mapM (translateExp unit locals) args
    return $ cCall cName cArgs

translateExp unit locals exp@ArrayCreate'{} = do
    (i, _) <- getAccumulator
    let name = createArrayMethodName i
    newDecl <- createArrayNewDecl unit locals i exp
    insertExtDecl newDecl
    return $ cCall name []
    
translateExp unit locals exp@ArrayCreateInit'{} = do
    (i, _) <- getAccumulator
    let name = createArrayMethodName i
    newDecl <- createArrayNewDecl unit locals i exp
    insertExtDecl newDecl
...
translateExp unit locals (MethodInv' (MethodCall' (pre:[name]) args))
    ...
    | otherwise = do
        ...
        let thisArg       = cVar (cIdent pre)
        cArgs <- mapM (translateExp unit locals) args
        return $ cCall cName (thisArg : cArgs)

translateExp unit locals (MethodInv' (MethodCall' [name] args)) = do 
    let cName = cIdent name
    cArgs <- mapM (translateExp unit locals) args
    return $ cCall cName cArgs

translateExp unit locals (MethodInv' (PrimaryMethodCall' exp name args)) = do
    let cName = cIdent name
    cExp  <- translateExp unit locals exp
    cArgs <- mapM (translateExp unit locals) args
    return $ cCall cName (cExp : cArgs)

translateExp unit locals (ArrayAccess' name indices) = do
    let cName  = cIdent name
    let cArray = cVar cName
    cIndices <- mapM (translateExp unit locals) indices
    let cAccess = foldl (\ acc -> cIndex (cMember acc arrayElementsName)) cArray cIndices
    return cAccess

translateExp unit locals (ExpName' name)
    = translateExpName unit locals name

translateExp unit locals (PostIncrement' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CPostIncOp cExp

translateExp unit locals (PostDecrement' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CPreDecOp cExp

translateExp unit locals (PreIncrement' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CPreIncOp cExp

translateExp unit locals (PreDecrement' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CPostDecOp cExp

translateExp unit locals (PrePlus' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CPlusOp cExp

translateExp unit locals (PreMinus' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CMinOp cExp

translateExp unit locals (PreBitCompl' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CCompOp cExp

translateExp unit locals (PreNot' exp) = do
    cExp <- translateExp unit locals exp
    return $ cUnary CNegOp cExp

translateExp unit locals (BinOp' exp1 op exp2) = do
    cExp1 <- translateExp unit locals exp1
    cExp2 <- translateExp unit locals exp2
    return $ cBinary cOp cExp1 cExp2
    where
        cOp = case op of
...
translateExp unit locals (Cond' guard exp1 exp2) = do
    cGuard <- translateExp unit locals guard
    cExp1  <- translateExp unit locals exp1
    cExp2  <- translateExp unit locals exp2
    return $ cCond cGuard cExp1 cExp2

translateExp unit locals (Assign' lhs op exp)  = do
    cLhs <- translateLhs unit locals lhs
    cExp <- translateExp unit locals exp
    return $ cAssign cOp cLhs cExp
    where
        cOp = case op of 
...
translateMaybeExp :: CompilationUnit' -> LocalInformation -> Maybe Exp' -> ExpAccumulator (Maybe CExpr)
translateMaybeExp unit locals Nothing    = return Nothing
translateMaybeExp unit locals (Just exp) = return <$> translateExp unit locals exp

translateExpName :: CompilationUnit' -> LocalInformation -> Name' -> ExpAccumulator CExpr
translateExpName unit (className, locals) names
    -- Case: the name is a local variable.
    | head names `elem` locals = do
        let cNames = map (cVar . cIdent) names
        return $ foldl1 cMemberVar cNames

    -- Case: the name is a field of the class.
    | Just thisClass <- findClass className unit
    , containsNonStaticFieldWithName thisClass (head names) = do 
        let cNames = map (cVar . cIdent) names
        return $ foldl cMemberVar thisVar cNames
...
translateFieldName :: CompilationUnit' -> ClassDecl' -> Name' -> ExpAccumulator CExpr
translateFieldName _ (ClassDecl' _ name _) (field:names) = do
    let cField = cVar (createStructName name field)
    let cNames = fmap (cVar . cIdent) names
    return $ foldl cMemberVar cField cNames
                
translateLhs :: CompilationUnit' -> LocalInformation -> Lhs' -> ExpAccumulator CExpr
translateLhs _ _ (Name' [name]) = return $ cVar (cIdent name)

translateLhs unit locals (Field' access)
    = translateFieldAccess unit locals access

translateLhs unit locals (Array' (ArrayIndex' array indices)) = do
    cArray   <- translateExp unit locals array
    cIndices <- mapM (translateExp unit locals) indices
    let cAccess = foldl ( \ acc -> cIndex (cMember acc arrayElementsName)) cArray cIndices
    return cAccess

translateFieldAccess :: CompilationUnit' -> LocalInformation -> FieldAccess' -> ExpAccumulator CExpr
translateFieldAccess unit locals (PrimaryFieldAccess' exp field) = do
    cExp <- translateExp unit locals exp
    return $ cMember cExp (cIdent field)

-- TODO: maybe an unsafe fromJust.
translateVarInits :: CompilationUnit' -> LocalInformation -> VarInits' -> ExpAccumulator CInit
translateVarInits unit locals inits = do
    cInits <- mapM (translateVarInit unit locals) inits
    return $ cArrayInit (map fromJust cInits)

translateVarInit :: CompilationUnit' -> LocalInformation -> VarInit' -> ExpAccumulator (Maybe CInit)
translateVarInit unit locals (InitExp' exp) = do
    cExp <- translateExp unit locals exp
    return $ Just $ cExpInit cExp

translateVarInit _ _ (InitArray' Nothing) = return Nothing

translateVarInit unit locals (InitArray' (Just inits)) = do
    cInits <- translateVarInits unit locals inits
    return $ Just cInits
...
createArrayNewDecl :: CompilationUnit' -> LocalInformation -> Int -> Exp' -> ExpAccumulator CExtDecl
createArrayNewDecl unit locals i exp = do
    let name     = createArrayMethodName i
        nameOfTy = createArrayTypeName dimensions ty
        returnTy = cStructType nameOfTy
        params   = cParams []
    body <- createArrayConstructorBody unit locals exp
    return $ cFunction returnTy name [params, cPointer] body
    where
        (ty, dimensions)
...
createArrayConstructorBody :: CompilationUnit' -> LocalInformation -> Exp' -> ExpAccumulator CStat
createArrayConstructorBody unit locals (ArrayCreate' ty sizes _) = do
    let nameOfTy  = createArrayTypeName dimensions ty
        ...
        cReturn   = cReturnStat (Just thisVar)
    init <- createInitForDimension unit locals 0 thisVar ty sizes Nothing
    return $ cCompoundStat [thisValue, cBlockStat init, cReturn]
    where
        dimensions = length sizes

createArrayConstructorBody unit locals (ArrayCreateInit' ty dimensions inits) = do
    let nameOfTy  = createArrayTypeName dimensions ty
        thisTy    = cStructType nameOfTy
        cSizes    = map (\ size -> snd $ runAccumulator (translateExp unit ("", []) size) (0, [])) sizes
        cReturn   = cReturnStat (Just thisVar)
        thisValue = cVarDeclStat (cDecl thisTy [(cDeclr thisName [cPointer], Just (cExpInit (cMalloc (cSizeofType thisTy []))))])
    initValue <- createArrayInitValue unit locals ty cSizes inits
    init      <- createInitForDimension unit locals 0 thisVar ty sizes (Just inits)
    return $ cCompoundStat [initValue, thisValue, cBlockStat init, cReturn]
    where
        sizes = sizesOfVarInit inits

createArrayInitValue :: CompilationUnit' -> LocalInformation -> Type' -> [CExpr] -> VarInits' -> ExpAccumulator CBlockItem
createArrayInitValue unit locals ty sizes inits = do
    let (cTy, tydDeclr) = translateType unit (Just ty)
        dDeclrs         = map cArray sizes ++ tydDeclr
        declr           = cDeclr (cIdent "initial") dDeclrs
    init <- translateVarInits unit locals inits
    return $ cVarDeclStat (cDecl cTy [(declr, Just init)])

createInitForDimension :: CompilationUnit' -> LocalInformation -> Int -> CExpr -> Type' -> [Exp'] -> Maybe VarInits' -> ExpAccumulator CStat
createInitForDimension unit locals depth expr ty [size] inits = do
    let (typeOfElem, dDeclr) = translateType unit (Just ty)
        sizeOfElem           = cSizeofType typeOfElem dDeclr
    sizeExpr <- translateExp unit locals size
    let setElems   = cExprStat $ cAssign CAssignOp elemsMember (cCalloc sizeExpr sizeOfElem)
    ...
    return $ cCompoundStat (setLength : setElems : setInitial)
    where
        lengthMember = cMember expr arrayLengthName
        elemsMember  = cMember expr arrayElementsName
    
createInitForDimension unit locals depth expr ty (size:sizes) inits = do
    let nameOfTy   = createArrayTypeName dimensions ty
    ...
    nextDim <- createInitForDimension unit locals (depth + 1) nextExpr ty sizes inits
    let forBody  = cCompoundStat [allocArray, cBlockStat nextDim]
        setInner = cForStat forInit forGuard forUpdate forBody
    return $ cCompoundStat [setLength, setElems, setInner]
    where
        lengthMember = cMember expr arrayLengthName
        elemsMember  = cMember expr arrayElementsName
        dimensions   = length sizes
